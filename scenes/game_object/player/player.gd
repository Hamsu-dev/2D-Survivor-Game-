...
func _process(delta):
	var movemnt_vector = get_movement_vector()
	var direction = movemnt_vector.normalized()
...
func _on_ability_upgrade_added(ability_upgrade: AbilityUpgrade, current_upgrades: Dictionary):
	...
	abilites.add_child(ability.ability_controller_scene.instantiate())
